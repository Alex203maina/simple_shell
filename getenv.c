 ...
 * Return: String array representation of the environment.
 */
char **get_environment_strings(info_t *info)
{
    if (!info->environment_strings || info->environment_changed)
    {
        info->environment_strings = list_to_strings(info->environment);
...
int remove_environment_variable(info_t *info, char *variable)
{
    ...
    while (node)
    {
        ...
        {
            info->environment_changed = delete_node_at_index(&(info->environment), index);
            index = 0;
